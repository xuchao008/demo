                ...
                <tr v-for="(item,index) in initItems" :key="item.id" v-show="show(item)" :class="{'child-tr':item.parent}">
                    <td v-for="(column,snum) in columns" :key="column.key" :style=tdStyle(column)>
                        ...
                        <div v-if="column.type === 'action'">
                            <!-- <Button :type="action.type" @click="RowClick(item,$event,index,action.text)" v-for='action in (column.actions)' :key="action.text">{{action.text}}</Button> -->
                            <Button @click="rowClickHandler($event, item, index)">编辑</Button>
                            <Button @click="rowClickHandler($event, item, index)">删除</Button>
                        </div>
